...
void trans4d::COMVEL(double &YLAT, double &YLON, int &JREGN, double &VN, double &VE, double &VU, double &SN, double &SE, double &SU)
{
    ...
    else if (JREGN >= 1 && JREGN <= NUMGRD)
    {
        ...
        VN = WEI[0][0] * VEL[0][0][0] + WEI[0][1] * VEL[0][1][0] + WEI[1][0] * VEL[1][0][0] + WEI[1][1] * VEL[1][1][0];

        VE = WEI[0][0] * VEL[0][0][1] + WEI[0][1] * VEL[0][1][1] + WEI[1][0] * VEL[1][0][1] + WEI[1][1] * VEL[1][1][1];

        VU = WEI[0][0] * VEL[0][0][2] + WEI[0][1] * VEL[0][1][2] + WEI[1][0] * VEL[1][0][2] + WEI[1][1] * VEL[1][1][2];

        SN = WEI[0][0] * STDEV[0][0][0] + WEI[0][1] * STDEV[0][1][0] + WEI[1][0] * STDEV[1][0][0] + WEI[1][1] * STDEV[1][1][0];

        SE = WEI[0][0] * STDEV[0][0][1] + WEI[0][1] * STDEV[0][1][1] + WEI[1][0] * STDEV[1][0][1] + WEI[1][1] * STDEV[1][1][1];

        SU = WEI[0][0] * STDEV[0][0][2] + WEI[0][1] * STDEV[0][1][2] + WEI[1][0] * STDEV[1][0][2] + WEI[1][1] * STDEV[1][1][2];
                     ...
                     double const &TWOPI, bool const &VERT, double &U1SS, double &U2SS, double &U3SS, double &U1DS, double &U2DS, double &U3DS)
{
    ...
    if (VERT)
    {
        ...
        F4 = -RATIO * Q / (R + DBAR);
        F3 = 0.5e0 * RATIO * (ETA / (R + DBAR) + YBAR * Q / ((R + DBAR) * (R + DBAR)) - DLOG(R + ETA));
    ...
    U2DS = -(YBAR * Q / (R * (R + PSI)) + CDIP * TERM - F1 * SDIP * CDIP) / TWOPI;
    U3DS = -(DBAR * Q / (R * (R + PSI)) + SDIP * TERM - F5 * SDIP * CDIP) / TWOPI;
    return;
}
...
void trans4d::PSDISP(double const &YLAT, double const &YLON, int const &MIN, double &DNORTH, double &DEAST, double &DUP)
{
    ...
    for (int K = 1; K <= PSGRID::numpsg; K++)
    {
        ...
        if (INSIDE)
        {
            ...
            double ANORTH = WEI[0][0] * AMP[0][0][0] + WEI[0][1] * AMP[0][1][0] + WEI[1][0] * AMP[1][0][0] + WEI[1][1] * AMP[1][1][0];
            double AEAST = WEI[0][0] * AMP[0][0][1] + WEI[0][1] * AMP[0][1][1] + WEI[1][0] * AMP[1][0][1] + WEI[1][1] * AMP[1][1][1];
            double AUP = WEI[0][0] * AMP[0][0][2] + WEI[0][1] * AMP[0][1][2] + WEI[1][0] * AMP[1][0][2] + WEI[1][1] * AMP[1][1][2];
                                      ...
                                      double &sx, double &sy, double &sz)
{
    ...
    sx = DSQRT(pow(slat * clon * sn, 2) + pow(slon * se, 2) + pow(clat * clon * su, 2));

    sy = DSQRT(pow(slat * slon * sn, 2) + pow(clon * se, 2) + pow(clat * slon * su, 2));
...
void trans4d::TOMNT(int const &IYR, int const &IMON, int const &IDAY, int &IHR, int &IMN, int &MINS)
{
    ...
    C = 365.25e0 * IYRP;
    D = 30.6001e0 * (IMOP + 1);
    MINS = (B + C + D + IDAY - 679006) * (24 * 60) + (60 * IHR) + IMN;
}
...
void trans4d::TOXYZ(double glat, double glon, double eht, double &x, double &y, double &z)
{
    ...
    //       implicit double precision(a-h,o-z)
    //       common/CONST/ a,f,e2,ep2,af,pi,twopi,rhosecf
    DECLARE_COMMON_CONST
